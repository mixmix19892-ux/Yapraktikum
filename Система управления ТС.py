def validate_fuel_consumption(method):
    ...
    """
    def wrapper(self: Vehicle, value):
        # вызов метода: method(self, value)
        if value <= 0:
            print("Ошибка: значение не может быть отрицательным.")
            return None

        fuel = method(self, value)

        if fuel == None:
            return None
        
        if fuel > self._current_fuel_level:
            print(f"Ошибка: недостаточно топлива для поездки. Нужно {fuel} л, " \
                  f"в наличии {self._current_fuel_level} л.")
            return None
        self._current_fuel_level -= fuel
        return fuel
...
class Vehicle:
    ...
    def display_info(self):
        ...
        """
        print(f"Название: {self._name}, Вместимость бака: {self._fuel_tank_capacity} л, " \
...
class Car(Vehicle):
    ...
    def calculate_fuel_consumption(self, distance):
        value = (distance / 100) * self._fuel_consumption_per_100km
        print(f"Расход на {distance} км: {value} л.")
        return value
...
class Airplane(Vehicle):
    ...
    def calculate_fuel_consumption(self, flight_time):
        value = flight_time * self._fuel_consumption_per_hour
        print(f"Расход за {flight_time} ч: {value:.2f} л.")
        return value
...
class Boat(Vehicle):
    ...
    def calculate_fuel_consumption(self, travel_time):
        value = travel_time * self._fuel_consumption_per_hour
        print(f"Расход за {travel_time} ч: {value:.2f} л.")
        return value
